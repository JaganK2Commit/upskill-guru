...
exports.login = async (req, res) => {
  ...
  if (await comparePassword(password, user.dataValues.Password)) {
    ...
    res
      .status(200)
      .send({ uid: user.get().UserId, roleId: user.RoleId, token });
  } else {
...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      userSkills = await db.userskills.bulkCreate(
        Skills.map((s) => ({ UserId: user.get().UserId, SkillId: s.SkillId }))
      );
    ...
    res
      .status(204)
      .send({ message: "error while updating database: " + error });
  }
...
exports.register = async (req, res) => {
  ...
  if (password !== confirmPassword) {
    ...
    res
      .status(449)
      .send({ message: "password does not match confirm password field" });
  }
  ...
  const token = jwt.sign({ sub: user.get().UserId }, "secret", {
    expiresIn: "7d",
  });
...
exports.getUserById = async (req, res) => {
  ...
  if (user) {
    ...
    // find user's skills
    const skills = await db.sequelize.query("CALL getUserSkills (:uid)", {
